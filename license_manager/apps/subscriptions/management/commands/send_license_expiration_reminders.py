...
class Command(BaseCommand):
    ...
    def _parse_enterprise_customer_uuids(self, uuids_string):
        """
        ...
        return [uuid.strip() for uuid in uuids if uuid.strip()]
    ...
    def _get_expiring_licenses(self, enterprise_customer_uuid, days_before_expiration):
        ...
        # Find the date range: we want licenses expiring around the target date
        # Use a 1-day window to allow for daily runs without duplicate sends
    ...
    def _send_expiration_reminder_email(self, license_obj, enterprise_customer):
        ...
        logger.info(
            ...
            f'expiring on {expiration_date.strftime("%Y-%m-%d")}'
        )
